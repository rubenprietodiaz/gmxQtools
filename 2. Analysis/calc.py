...
# Input
pdb_file = 'hexagon.pdb'
ref_pdb_file = 'ref.pdb'  
smarts_pattern = '[#8]-[#6]-[#7]-[#6]1-[#6](-[#6]-[#6](-[#6]-[#6]-1-[#9])-[#6](-[#8])-[#7]-[#6]1-[#7]-[#6]-[#6]-[#16]-1)-[#9]'
ligand_name = 'L01'
ligand_pdb = f'{ligand_name}.pdb'
ref_pdb = 'ref_lig.pdb'
traj_file = 'traj_centered_to_start.xtc'
xvg_filename_rmsd = 'rmsd.xvg'
xvg_filename_rmsf = 'rmsf.xvg'
output_filename_rmsd = 'rmsd_results_smi.txt'
output_filename_rmsf = 'rmsf_results_smi.txt'
selection_pattern = 'invert' # 'invert' or 'select
...
def return_non_matching_atom_numbers(smarts, ligand_pdb):
    '''Returns the atom numbers in a ligand PDB file that do not match the SMARTS pattern.'''
    mol_from_pdb = Chem.MolFromPDBFile(ligand_pdb)
    mol_from_smarts = Chem.MolFromSmarts(smarts)
    matching_atoms = set([atom for match in mol_from_pdb.GetSubstructMatches(mol_from_smarts) for atom in match])
    all_atoms = set(range(mol_from_pdb.GetNumAtoms()))
    non_matching_atoms = list(all_atoms - matching_atoms)

    # Adjusting atom numbers for PDB file which might be 1-based
    with open(ligand_pdb, 'r') as file:
        lines = file.readlines()
    atom_numbers = [int(lines[atom].split()[1]) - 1 for atom in non_matching_atoms]  # Convert 1-based to 0-based
    return atom_numbers
...
for subdir in os.listdir('.'):
    ...
    if selection_pattern == 'invert':
        lig_atom_numbers = return_non_matching_atom_numbers(smarts_pattern, ligand_pdb)
    else:
